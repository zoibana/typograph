...
{
	public const LAYOUT_STYLE = 1;
	public const LAYOUT_CLASS = 2;

	public const INTERNAL_BLOCK_OPEN = '%%%INTBLOCKO235978%%%';
	...
	public static array $_charsTable = [
		'"' => [
			'html' => [
				'&laquo;', '&raquo;', '&rdquo;', '&lsquo;', '&bdquo;', '&ldquo;', '&quot;', '&#171;', '&#187;'
			],
			'utf8' => [
				0x201E, 0x201C, 0x201F, 0x201D, 0x00AB, 0x00BB
			]
		],
		' ' => ['html' => ['&nbsp;', '&thinsp;', '&#160;'],
	...
	protected static array $html4_char_ents = [
		'nbsp' => 160,
		'iexcl' => 161,
		'cent' => 162,
		'pound' => 163,
	 ...
	 */
	protected static ?string $tagId = null;
	 ...
	 * @author    somebody?
	 */
	public static function getUnicodeChar(int $c)
	{
		if ($c <= 0x7F) {
			return chr($c);
		}
	 ...
	 * <code>
	 *  // Remove UTF-8 chars:
	 *    $str = static::clear_special_chars('your text', 'utf8');
	 *  // ... or HTML codes only:
	 *    $str = static::clear_special_chars('your text', 'html');
	 *    // ... or combo:
	 *  $str = static::clear_special_chars('your text');
	 * </code>
	 */
	public static function clearSpecialChars(string $text, $mode = null)
	{
		if (is_string($mode)) {
			$mode = [$mode];
		}
				...
				if (isset($vals[$type])) {
					foreach ($vals[$type] as $v) {
						if ('utf8' === $type && is_int($v)) {
							$v = self::getUnicodeChar($v);
						}

						if (('html' === $type) && preg_match("/<[a-z]+>/i", $v)) {
							$v = self::safeTagChars($v, true);
						}
	 ...
	 */
	public static function safeTagChars(string $text, $way): string
	{
		if ($way) {
			$text = preg_replace_callback(
				'/(<\/?)([^<>]+?)(>)/',
				static function ($m) {
					return (strlen($m[1]) === 1 && strpos(trim($m[2]), '-') === 0 && substr(trim($m[2]), 1, 1) !== '-') ? $m[0] : $m[1] . (strpos(trim($m[2]), "a") === 0 ? "%%___" : "") . static::encryptTag(trim($m[2])) . $m[3];
				},
				$text
			);
		} else {
			$text = preg_replace_callback(
				'/(<\/?)([^<>]+?)(>)/',
				static function ($m) {
					return (strlen($m[1]) === 1 && strpos(trim($m[2]), '-') === 0 && substr(trim($m[2]), 1, 1) !== '-') ? $m[0] : $m[1] . (strpos(trim($m[2]), "%%___") === 0 ? static::decryptTag(substr(trim($m[2]), 4)) : static::decryptTag(trim($m[2]))) . $m[3];
				},
				$text
			);
		}
	 ...
	 * @return  string
	 */
	public static function decodeInternalBlocks(string $text): string
	{
		return preg_replace_callback(
			'#' . static::INTERNAL_BLOCK_OPEN . '([a-zA-Z0-9/=]+?)' . static::INTERNAL_BLOCK_CLOSE . '#s',
			static function ($m) {
				return static::decryptTag($m[1]);
			},
			$text
		);
	}
	 ...
	 * Кодирует спец блок
	 */
	public static function encodeBlock(string $text): string
	{
		return static::INTERNAL_BLOCK_OPEN . static::encryptTag($text) . static::INTERNAL_BLOCK_CLOSE;
	}
	 ...
	 * @throws \Exception
	 */
	public static function buildSafeTag(string $content, string $tag = 'span', array $attribute = [], $layout = self::LAYOUT_STYLE): string
	{
		$htmlTag = $tag;

		if (self::$tagId && !isset($attribute['id'])) {
			$attribute['id'] = 'emt-2' . random_int(1000, 9999);
		}

		$classname = "";
		if (count($attribute)) {

			if (($layout & static::LAYOUT_STYLE) && isset($attribute['__style']) && $attribute['__style']) {
		...
		if (($layout & static::LAYOUT_CLASS) && $classname) {
			$htmlTag .= " class=\"$classname\"";
		}

		return "<" . self::encryptTag($htmlTag) . ">$content</" . self::encryptTag($tag) . ">";
	}
	 ...
	 * @return    string
	 */
	public static function encryptTag(string $text): string
	{
	 ...
	 * @return    string
	 */
	public static function decryptTag(string $text): string
	{
	 ...
	 * @return string
	 */
	public static function html_char_entity_to_unicode(string $entity)
	{
		if (isset(self::$html4_char_ents[$entity])) {
			return self::getUnicodeChar(self::$html4_char_ents[$entity]);
		}
